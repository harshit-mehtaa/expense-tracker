...
## Series plan (9 /task runs total, user-approved order) — 8 of 9 done:
...
9. [QUEUED NEXT] Dashboard.test.tsx coverage check + gap-fill (final task)

## Tech debt inventory (see git log for full task-by-task detail):
- `resolveTargetUserId` (backend) only checks `deletedAt`, not
  `isActive` — a broader, pre-existing gap affecting every caller, not
  just Task 8's new deep link (which now validates against loaded
  members client-side as a local mitigation). Needs a backend audit.
- `spendingByCat` query in Reports.tsx has no isError handling, unlike
  its P&L/Trial Balance siblings in the same file — general gap.
...
- `CashflowMonth`/`UpcomingAlert` types independently duplicated
  (api/dashboard.ts + shared/types/index.ts).
- `useAccounts`/`useCategories` independently duplicated 5-way (same
  query key, separate definitions).
...
- No modal in the app has role="dialog"/focus-trap/Escape-to-close (12
  modals checked). Chart click-handlers (cashflow, family spending) are
  mouse-only, no keyboard equivalent — systemic a11y gap.
