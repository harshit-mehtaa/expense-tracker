...
export const taxApi = {
  ...
  get80CTracker: (fy: string, viewUserId?: string) =>
    api.get<{ data: any }>(`/tax/80c-tracker?fy=${fy}${uid(viewUserId)}`).then(unwrap),
  getAdvanceTaxCalendar: (fy: string) =>
    api.get<{ data: any[] }>(`/tax/advance-tax-calendar?fy=${fy}`).then(unwrap),
  ...
  getCapitalGainsSummary: (fy: string, viewUserId?: string) =>
    api.get<{ data: CapitalGainSummary }>(`/tax/capital-gains/summary?fy=${fy}${uid(viewUserId)}`).then(unwrap),
  ...
  getOtherIncomeSummary: (fy: string, viewUserId?: string) =>
    api.get<{ data: OtherIncomeSummary }>(`/tax/other-income/summary?fy=${fy}${uid(viewUserId)}`).then(unwrap),
  ...
  getHousePropertySummary: (fy: string, viewUserId?: string) =>
    api.get<{ data: HousePropertyIncomeSummary }>(`/tax/house-property/summary?fy=${fy}${uid(viewUserId)}`).then(unwrap),
  ...
  getForeignAssetSummary: (fy: string, viewUserId?: string) =>
    api.get<{ data: ForeignAssetSummary }>(`/tax/foreign-assets/summary?fy=${fy}${uid(viewUserId)}`).then(unwrap),

  // ─── ITR-2 Overview ────────────────────────────────────────────────────────
  getITR2Summary: (fy: string, viewUserId?: string) =>
    api.get<{ data: ITR2Summary }>(`/tax/itr2-summary?fy=${fy}${uid(viewUserId)}`).then(unwrap),
};
